...
node("${ENV}") {
    ...
    def HOTFIX_ARTIFACT_NAME   = ""
    def FILE_LIST_PATH         = "${workspacePath}/hotfix_filelist.txt"
    def DELETED_FILE_LIST_PATH = "${workspacePath}/hotfix_deleted_filelist.txt"
    ...
    try {
        ...
        stage('Fetch Changed Files') {
            dir('eximbills') {
                script {
                            ...
                            nameStatus.split('\n').each { line ->
                                ...
                                if (parts.size() == 2) {
                                    ...
                                    if (status == 'A') {
                                        ...
                                        echo "Skipping unsupported Git status '${status}' for ${filePath}"
                                    }
                                }
                            }
                        }
                    ...
                    deletedFiles = deletedFiles.unique()

                    // Deleted files must not be placed inside the ZIP.
                    addedFiles = addedFiles.findAll { !deletedFiles.contains(it) }
                    modifiedFiles = modifiedFiles.findAll { !deletedFiles.contains(it) }

                    def allChangedFiles = (addedFiles + modifiedFiles).unique()

                    if (allChangedFiles.size() == 0 && deletedFiles.size() == 0) {
                        error "No changed files found under 'jboss/' for PR #${params.PR_NUMBER}"
                    }

                    // Added/modified files are packaged.
                    writeFile file: FILE_LIST_PATH, text: allChangedFiles.join('\n')

                    // Deleted files are written separately for deployment-side removal.
                    writeFile file: DELETED_FILE_LIST_PATH, text: deletedFiles.join('\n')
                    ...
                    echo "Deleted files  : ${deletedFiles.size()}"
                    echo "Total packaged : ${allChangedFiles.size()}"
                    echo "============================================================"
                    ...
                    if (deletedFiles.size() > PRINT_THRESHOLD) {
                        echo "Deleted files (${deletedFiles.size()}) - PRINT_THRESHOLD more (truncated)"
                    } else {
                        deletedFiles.each { f -> echo "[DEL] ${f}" }
                    }

                    echo "============================================================"
                    echo "Total packaged files : ${allChangedFiles.size()}"
                    echo "Total deleted files  : ${deletedFiles.size()}"
                    echo "============================================================"
        ...
        stage('Create Hotfix Archive') {
            script {
                ...
                ansiblePlaybook(
                    ...
                    extras: "-i playbooks/env/${ENVNAME} -e ENVNAME=${ENVNAME} -e WRK=${workspacePath} -e filelist_path=${FILE_LIST_PATH} -e deleted_filelist_path=${DELETED_FILE_LIST_PATH} -e artifact_name=${HOTFIX_ARTIFACT_NAME} -t archive"
                )
            }
        }
        ...
        stage('Copy to Precheck Folder') {
            ansiblePlaybook(
                playbook: "playbooks/EXB-Hotfix-Build.pb",
                extras: "-i playbooks/env/${ENVNAME} -e ENVNAME=${ENVNAME} -e WRK=${workspacePath} -e filelist_path=${FILE_LIST_PATH} -e deleted_filelist_path=${DELETED_FILE_LIST_PATH} -e artifact_name=${HOTFIX_ARTIFACT_NAME} -t precheck"
            )
